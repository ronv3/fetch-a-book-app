...
async function listiraamatud() {
  try {
    const responseData = await getDataAsJson("https://fetch-books-backend-atfma3ccece9bma5.northeurope-01.azurewebsites.net/raamatud/");
    ...
    for (const filename of responseData.raamatud) {
      ...
      const downloadUrl = `https://fetch-books-backend-atfma3ccece9bma5.northeurope-01.azurewebsites.net/raamatud/${raamatId}`;
